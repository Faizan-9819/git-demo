...
const TENANT_SLUG = "growth-rocket";
const SITE_SLUG = "growth-rocket";
...
function formatPrice(priceMinor?: number | null, currency?: string | null) {
  ...
  try {
    ...
    return `${priceMinor} ${currency ?? ""}`.trim();
  }
}
...
export default function BookingForm({ isOpen, onClose }: BookingFormProps) {
  ...
  }, [isOpen, step, serviceId, dateRange]);
  ...
  const handleSubmit = useCallback(async () => {
    if (!serviceId || !selectedSlot || !name.trim()) {
      setSubmitError(
        t({
          en: "Please choose a service, a time, and enter your name before submitting.",
          nl: "Kies een dienst, een tijd en vul je naam in voordat je verzendt.",
        }),
      );
      return;
    }
    if (honeypot) {
      // Spam bots fill hidden fields; pretend success so they don't learn to skip it.
      goTo("success");
      return;
    }
    setSubmitting(true);
  ...
  }, [serviceId, selectedSlot, name, email, phone, notes, honeypot, goTo]);
      ...
      {isOpen && (
        <>
          <motion.div
            ...
            onClick={onClose}
            className="fixed inset-0 z-[100] bg-[var(--color-haiti)]/50 backdrop-blur-sm"
          />

          <div className="fixed inset-0 z-[101] flex items-center justify-center p-3 lg:p-6 pointer-events-none">
            <motion.div
              initial={{ opacity: 0, scale: 0.96, y: 24 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.96, y: 24 }}
              transition={{ type: "spring", duration: 0.55, bounce: 0.28 }}
              className="relative w-full max-w-[600px] bg-white rounded-3xl shadow-2xl pointer-events-auto overflow-hidden flex flex-col max-h-[calc(100vh-24px)]"
              role="dialog"
              aria-modal="true"
              aria-label={t({
            ...
            >
              {/* Header */}
              <div
                className="relative shrink-0 px-6 lg:px-8 pt-5 pb-5 text-white overflow-hidden"
                style={{ background: "var(--grad-brand)" }}
              >
                <div
                  aria-hidden
                  className="pointer-events-none absolute -top-24 -right-16 w-[320px] h-[320px] rounded-full opacity-30"
                  style={{
                    background:
                      "radial-gradient(50% 50% at 50% 50%, rgba(254,229,102,0.55) 0%, rgba(254,229,102,0) 70%)",
                  }}
                />
                <div
                  aria-hidden
                  className="pointer-events-none absolute -bottom-20 -left-10 w-[260px] h-[260px] rounded-full opacity-30"
                  style={{
                    background:
                      "radial-gradient(50% 50% at 50% 50%, rgba(192,132,252,0.6) 0%, rgba(192,132,252,0) 70%)",
                  }}
                />
                <div className="relative flex items-start justify-between gap-4">
                  <div>
                    <p className="font-poppins text-[11px] uppercase tracking-[2px] text-white/70">
                      {step === "success"
                    ...
                    <h2 className="font-poppins font-semibold text-[clamp(20px,3.8vw,24px)] leading-[1.15] mt-1">
                      {step === "service" &&
                  ...
                  <button
                    ...
                    className="shrink-0 -m-2 p-2 rounded-full text-white/80 hover:text-white hover:bg-white/10 transition-colors cursor-pointer"
                  >
                    <svg
                      width="20"
                      height="20"
                  ...
                  <div className="relative mt-4 flex items-center gap-2">
                    {[1, 2, 3].map((n) => {
                      ...
                      return (
                        <div
                          key={n}
                          className="flex-1 h-[5px] rounded-full overflow-hidden bg-white/15"
                        >
                          <motion.div
                            initial={false}
              ...
              {/* Body — fixed min-height so every step lines up */}
              <div className="relative px-6 lg:px-8 py-6 min-h-[480px] flex flex-col">
...
function ServiceStep({
  ...
  return (
    <div className="flex flex-col gap-4 flex-1">
      ...
      {!loading && !error && (
        <>
          <p className="font-inter text-[13px] text-[var(--color-dolphin)] -mt-1">
            {t({
              en: "Choose what you'd like to book.",
              nl: "Kies welke dienst je wilt boeken.",
            })}
          </p>

          <div className="services-list grid grid-cols-1 gap-2.5 overflow-y-auto max-h-[340px] -mx-1 px-1 pb-1" data-lenis-prevent>
            {services.length === 0 && (
              <div className="rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 text-[13px] text-slate-600">
                {t({
            ...
            {services.map((s) => {
              ...
              const price = s.isConsultation
                ? null
                : formatPrice(s.priceMinor, s.currencyCode);
              return (
                <button
                  type="button"
                  key={s.id}
                  ...
                  className={
                    ...
                    (active
                      ? "border-[var(--color-violet-42)] bg-[var(--color-violet-98)] shadow-[0_8px_24px_-12px_rgba(91,33,182,0.35)]"
                      : "border-slate-300 hover:border-[var(--color-violet-76)] hover:bg-[var(--color-violet-98)]/40")
                  }
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      {s.description && (
                        <div className="mt-0.5 font-inter text-[12.5px] leading-[1.45] text-[var(--color-dolphin)] line-clamp-2">
                          {s.description}
                        </div>
                      )}
                      <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 font-inter text-[12px] text-[var(--color-slate-500)]">
                        ...
                        {price && (
                          <span className="inline-flex items-center gap-1 text-[var(--color-violet-42)] font-medium">
                            {price}
                          </span>
                        )}
                      </div>
    ...
    <div className="flex flex-col gap-5 flex-1">
      {loading && (
        <div className="flex flex-col items-center justify-center gap-3 py-10 flex-1">
          <Spinner />
          <p className="font-inter text-[13px] text-[var(--color-dolphin)]">
            {t({
              en: "Finding available times…",
              nl: "Beschikbare tijden worden gezocht…",
            })}
          </p>
        </div>
      )}
      ...
      {!loading && !error && (
        <>
          ...
          <div>
            {selectedDate && (
              <p className="mb-1 font-inter font-semibold text-[13px] text-[var(--color-haiti)]">
            ...
            <p className="mb-2 font-inter text-[11.5px] uppercase tracking-[1.4px] text-[var(--color-slate-500)] font-semibold">
              {t({ en: "Available times", nl: "Beschikbare tijden" })}{" "}
              {selectedDate && (
                <span className="text-[var(--color-haiti)] normal-case tracking-normal font-medium">
            ...
            {!selectedDate ? (
              <div className="rounded-xl border border-slate-300 bg-slate-50 px-4 py-5 text-center">
                <p className="font-inter text-[13px] text-[var(--color-slate-600)]">
                  {t({
              ...
              <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-5 text-center">
                <p className="font-inter text-[13px] text-amber-800">
                  {t({
                ...
                {availableSlots.map((slot) => {
                  ...
                  return (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.96 }}
                      className={
                        "shrink-0 rounded-xl border px-4 py-2.5 font-inter font-medium text-[13px] whitespace-nowrap transition-colors cursor-pointer " +
                        (active
                          ? "border-[var(--color-violet-42)] bg-[var(--color-violet-42)] text-white shadow-[0_8px_20px_-10px_rgba(91,33,182,0.6)]"
                          : "border-slate-300 bg-white text-[var(--color-haiti)] hover:border-[var(--color-violet-76)] hover:bg-[var(--color-violet-98)]/40")
                      }
                    >
...
function Calendar({
  ...
  return (
    <div className="rounded-2xl border border-slate-300 bg-white p-3 lg:p-4">
      <div className="flex items-center justify-between mb-3">
        <button
          ...
          className="inline-flex items-center justify-center size-[32px] rounded-lg border border-slate-300 text-[var(--color-haiti)] hover:border-[var(--color-violet-76)] hover:bg-[var(--color-violet-98)]/40 disabled:opacity-30 disabled:cursor-not-allowed transition-colors cursor-pointer"
        >
          <svg
            width="14"
            height="14"
        ...
        <div className="font-poppins font-semibold text-[14.5px] text-[var(--color-haiti)]">
          {monthLabel}
        </div>
        <button
          type="button"
      ...
      <div className="grid grid-cols-7 gap-1 mb-1">
        {(locale === "nl"
          ...
          <div
            key={i}
            className="text-center font-inter text-[10.5px] uppercase tracking-[1px] text-[var(--color-slate-500)] py-1"
          >
            {d}
          </div>
        ))}
        ...
        {cells.map((cell, i) => {
          ...
          return (
            <button
              ...
              className={
                "relative h-9 rounded-lg font-inter text-[13px] transition-all cursor-pointer " +
                (active
                  ? "bg-[var(--color-violet-42)] text-white font-semibold shadow-[0_8px_20px_-10px_rgba(91,33,182,0.6)]"
                  : available && inRange
                    ? "bg-white text-[var(--color-haiti)] hover:bg-[var(--color-violet-98)] hover:text-[var(--color-violet-42)] font-medium"
            ...
            >
              ...
              {!active && available && inRange && (
                <span
                  aria-hidden
                  className="absolute left-1/2 -translate-x-1/2 bottom-[3px] size-[3px] rounded-full bg-[var(--color-violet-42)]"
                />
              )}
              {!active && isToday && (
                <span
                  aria-hidden
                  className="absolute inset-[2px] rounded-lg ring-1 ring-[var(--color-violet-76)] pointer-events-none"
                />
              )}
            </button>
          );
...
function DetailsStep({
  ...
  return (
    ...
    >
      ...
      {service && slot && (
        <div className="flex items-center gap-2.5 rounded-xl bg-[var(--color-violet-98)]/60 border border-[var(--color-violet-92)] px-3 py-2.5">
          <span
            className="inline-flex items-center justify-center size-[32px] rounded-lg text-white shrink-0"
            style={{ background: "var(--grad-brand)" }}
          >
            <svg
              width="15"
          ...
          <div className="min-w-0 flex-1">
            <div className="font-poppins font-medium text-[13px] text-[var(--color-haiti)] truncate">
              {service.name}
            </div>
            <div className="font-inter text-[12px] text-[var(--color-dolphin)] truncate">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <Field label={t({ en: "Phone", nl: "Telefoonnummer" })}>
        ...
        <div className="lg:col-span-2">
          <Field label={t({ en: "Email", nl: "E-mailadres" })}>
            <div className="space-y-1">
              <Input
                type="email"
                value={email}
        ...
        <div className="lg:col-span-2">
          ...
          >
            <textarea
              ...
              rows={3}
              className="w-full px-4 py-3 rounded-xl border border-slate-300 focus:border-[var(--color-violet-42)] focus:ring-2 focus:ring-[var(--color-violet-42)]/20 outline-none transition-all font-inter text-[14px] text-[var(--color-haiti)] resize-none"
            />
          </Field>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center gap-4 py-2 flex-1">
      <motion.div
        ...
        className="size-[68px] rounded-full flex items-center justify-center text-white shadow-[0_18px_40px_-12px_rgba(34,160,107,0.5)]"
        style={{
          background:
            "linear-gradient(135deg, var(--color-green-500) 0%, var(--color-green-700) 100%)",
        }}
      >
        <motion.svg
      ...
      <div className="flex flex-col gap-1.5">
        <h3 className="font-poppins font-semibold text-[22px] leading-[1.2] text-[var(--color-haiti)]">
          {t({ en: "Thanks", nl: "Bedankt" })}
          {name ? `, ${name.split(" ")[0]}` : ""}!
        </h3>
        <p className="font-inter text-[14px] leading-[1.5] text-[var(--color-dolphin)] max-w-[420px]">
          {t({
      ...
      {(service || slot) && (
        <div className="w-full max-w-[420px] mt-2 rounded-2xl border border-slate-300 bg-white px-4 py-3 text-left">
...
function Field({
  ...
  return (
    <label className="flex flex-col gap-1.5">
      <span className="font-inter font-medium text-[12.5px] text-[var(--color-slate-700)]">
        {label}
      </span>
      {children}
    </label>
...
function Input({
  ...
  return (
    <input
      {...props}
      className={
        "w-full h-[46px] px-4 rounded-xl border focus:ring-2 outline-none transition-all font-inter text-[14px] text-[var(--color-haiti)] " +
        (error
          ? "border-red-500 focus:border-red-500 focus:ring-red-500/20"
          : "border-slate-300 focus:border-[var(--color-violet-42)] focus:ring-[var(--color-violet-42)]/20") +
        (props.placeholder ? " placeholder:text-slate-400" : "") +
        " " +
        (props.className ?? "")
      }
    />
...
function SummaryRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col">
      <span className="font-inter text-[11px] uppercase tracking-[1px] text-[var(--color-slate-500)]">
        {label}
      </span>
      <span className="font-inter font-medium text-[13.5px] text-[var(--color-haiti)] mt-0.5">
        {value}
      </span>
    </div>
  );
...
function Spinner({ small = false }: { small?: boolean }) {
  ...
  return (
    <span
      className="inline-block animate-spin rounded-full border-[2.5px] border-[var(--color-violet-92)] border-t-[var(--color-violet-42)]"
