...
export default function BlogLandingClient({ posts }: { posts: BlogPost[] }) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Filter Bar */}
      <div
        className="py-[18px] lg:py-5 lg:h-fit flex items-center justify-center px-4 sm:px-6 plcont prcont"
        style={{
          background:
          ...
          <div className="flex flex-wrap items-center gap-[8px] lg:gap-[10px]">
            ...
            <div className="relative">
              <button
                ...
                className="bg-black text-white rounded-[999px] px-[19px] py-[11px] flex items-center gap-[8px]  font-semibold text-[13px] hover:bg-gray-900 transition-colors"
              >
                {ui.industries}
                <svg
                  className={`w-[11px] h-[6.5px] transition-transform ${isDropdownOpen ? "rotate-180" : ""}`}
                  fill="none"
                  ...
                  >
                    {dropdownCategories.map(({ name }) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2.5 hover:bg-gray-50 transition-colors text-[14px] border-b border-[#ece8f5] last:border-b-0 ${selectedCategory === name ? "bg-[#f2eefc] text-[#6b2cc9] font-semibold" : "text-[#6b6880]"}`}
                      >
                        {name}
                      </button>
                    ))}
            ...
            {allAcfCategories.slice(0, 4).map((cat) => (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className={`bg-white rounded-[999px] px-[19px] py-[7px] flex items-center gap-[8px] border border-[#e3dff0] font-semibold text-[13px] transition-all ${selectedCategory === cat.name ? "ring-2 ring-[#6b2cc9]" : ""}`}
              >
                <span className="text-[#1a1530]">{cat.name}</span>
                <span className="text-[#9a97ab] text-[11px] font-medium">
                  · {cat.count}
                </span>
              </motion.button>
            ))}
